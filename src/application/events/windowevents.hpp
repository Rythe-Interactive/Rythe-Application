...
namespace rythe::application
{
	struct window_close final : public events::event<window_close>
	{
		...
		window_close(ecs::component<window> windowHandle)
	...
	struct window_move final : public events::event<window_move>
	{
		...
		window_move() = default;
		window_move(ecs::component<window> windowHandle, math::int2 position)
			: windowHandle(windowHandle),
			  position(position)
		{
		}
	...
	struct window_resize final : public events::event<window_resize>
	{
		...
		window_resize(ecs::component<window> windowHandle, math::int2 size)
			: windowHandle(windowHandle),
			  size(size)
		{
		}
	...
	struct window_refresh final : public events::event<window_refresh>
	{
		...
		window_refresh(ecs::component<window> windowHandle)
			: windowHandle(windowHandle)
		{
		}
	};
	...
	struct window_focus final : public events::event<window_focus>
	{
		...
		window_focus(ecs::component<window> windowHandle, int focused)
			: windowHandle(windowHandle),
			  focused(focused)
		{
		}
	...
	struct window_iconified final : public events::event<window_iconified>
	{
		...
		window_iconified(ecs::component<window> windowHandle, int iconified)
			: windowHandle(windowHandle),
			  iconified(iconified)
		{
		}
	...
	struct window_maximized final : public events::event<window_maximized>
	{
		...
		window_maximized(ecs::component<window> windowHandle, int maximized)
			: windowHandle(windowHandle),
			  maximized(maximized)
		{
		}
	...
	struct window_framebuffer_resize final : public events::event<window_framebuffer_resize>
	{
		...
		window_framebuffer_resize(ecs::component<window> windowHandle, math::int2 size)
			: windowHandle(windowHandle),
			  size(size)
		{
		}
	...
	struct window_content_rescale final : public events::event<window_content_rescale>
	{
		...
		window_content_rescale(ecs::component<window> windowHandle, math::float2 scale)
			: windowHandle(windowHandle),
			  scale(scale)
		{
		}
	...
	struct window_item_dropped final : public events::event<window_item_dropped>
	{
		...
		window_item_dropped(ecs::component<window> windowHandle, int count, rsl::cstring* cpaths)
			: windowHandle(windowHandle)
		{
	...
	struct mouse_enter_window final : public events::event<mouse_enter_window>
	{
		...
		mouse_enter_window(ecs::component<window> windowHandle, int entered)
			: windowHandle(windowHandle),
			  entered(entered)
		{
		}
